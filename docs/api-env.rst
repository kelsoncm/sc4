...
All functions share an optional `wrapped` parameter: when `True`, leading and trailing
single-quotes are stripped from the value (useful for values set as `'foo'` in shell scripts).
